...
import { Eye, Loader2, PlusCircle, Save, Trash2, Upload, Camera, Palette, Layout as LayoutIcon, PanelLeft, ImageIcon, Youtube, RectangleHorizontal, RectangleVertical, PictureInPicture2, Lock } from 'lucide-react';
...
const Home = () => {
  ...
  const [subscriptionPlan, setSubscriptionPlan] = useState<'free' | 'pro'>('free'); // Yeni eklenen state
    ...
    const fetchData = async () => {
      ...
      if (user) {
        ...
        if (profileResponse.error) {
          ...
          setSubscriptionPlan(data.subscription_plan || 'free'); // Abonelik planını ayarla
        }
        ...
        <Card className="mb-8 bg-gradient-to-r from-indigo-500 to-purple-600 text-white">
          <CardContent className="p-6">
            <div className="flex flex-col md:flex-row justify-between items-center gap-4">
              <div>
                <h3 className="text-xl font-bold">Pro Plan'a Yükselt</h3>
          ...
          <form onSubmit={handleAddLink}>
            <CardContent className="space-y-4">
              <div className="space-y-2">
                ...
                <Input id="title" placeholder<dyad-write path="src/pages/Home.tsx" description="Kullanıcının abonelik planına göre pro özellikleri kilitleme ve link sınırı uygulama">
import { useEffect, useState, useRef } from 'react';
import { Link, useNavigate } from 'react-router-dom';
import { supabase } from '@/lib/supabase';
import { Button } from '@/components/ui/button';
